function [chii]=match_end(template1,template2,noShow) 
...
f2=(template2);
f2=f2.f;
chii=chi_square_statistics(f1,f2)
d=1-f1*f2'/(norm(f1)*norm(f2));
if nargin == 2 
    
   %text=strcat('The max matching percentage is  ',num2str(percent_match),'%'); 
   if chii<=2.0700e+04 && d<.2
       disp 'jjjj'
   h = msgbox('MATCHED', 'Success');
